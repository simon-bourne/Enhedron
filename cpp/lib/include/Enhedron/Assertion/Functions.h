    ...
    return makeFunction(
        "countMatching", [](auto&& container, auto&& predicate) {
            return count_if(
                container.begin(),
                container.end(),
                forward<decltype(predicate)>(predicate));
...
auto startsWith(MainContainer&& container, StartsWithContainer&& starting)
{
    return makeFunction(
        ...
        forward<MainContainer>(container),
        forward<StartsWithContainer>(starting));
}
...
auto endsWith(MainContainer&& container, EndsWithContainer&& ending)
{
    return makeFunction("endsWith", [](auto&& container, auto&& ending) {
        ...
        auto size =
            static_cast<typename ContainerType::difference_type>(ending.size());
