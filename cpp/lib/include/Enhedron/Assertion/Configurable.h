...
struct Convert<Container, std::enable_if_t<IsStlContainer<Container>::value>>
{
    static inline std::string toString(const Container& value)
    {
        ...
        for (const auto& element : value)
        {
            ...
            isFirst = false;
            result +=
                Convert<typename Container::value_type>::toString(element);
        }
    ...
    ResultType evaluate()
    {
        return extractParameterPack(
            [this](Args&&... extractedArgs) {
                return functor(
                    getParameterValue(forward<Args>(extractedArgs))...);
            },
            move(args));
    }
...
public:
    ...
    explicit VariableRefExpression(
...
public:
    ...
    explicit VariableValueExpression(
        const char* variableName,
        Value&& value,
        const char* file,
        int line)
...
template <typename Operation,
          typename Lhs,
          typename Rhs,
          pair<IsExpression<Lhs>, IsExpression<Rhs>>* = nullptr>
auto apply(
...
template <typename Operation,
          typename Lhs,
          typename Rhs,
          pair<IsExpression<Lhs>, IsNotExpression<Rhs>>* = nullptr>
auto apply(
    const char* operationName,
    Operation operation,
    const Lhs& lhs,
...
template <typename Operation,
          typename Lhs,
          typename Rhs,
          pair<IsNotExpression<Lhs>, IsExpression<Rhs>>* = nullptr>
auto apply(
    const char* operationName,
    Operation operation,
    const Lhs& lhs,
    ...
    ContextVariableList... contextVariableList)
{
    failureHandler->fail(
        move(description),
        expression.makeName(),
        buildVariableList(expression, contextVariableList...));
}
...
template <typename Exception, typename... Tail>
bool CheckThrowsWithFailureHandler(
    Out<FailureHandler> failureHandler,
    string description,
    Tail... tail)
...
#define M_ENHEDRON_VAR(expression)                                             \
    (::Enhedron::Assertion::Impl::Configurable::makeVariable(                  \
