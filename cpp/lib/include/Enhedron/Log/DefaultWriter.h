...
public:
     ...
     *
     * { "utcTime": "2014-03-02 12:19:12.580420", "level": "Error", "threadId":
     * "12345", "module": "MyModule", "message": "File not found", "file":
     * "/home/myuser/myfile" }
     */
        ...
        const VarField&... extraFields)
    {
        ...
        writeMember(
            outStream, "type", EscapedCharString{logEntryTypeToString(type)});
...
private:
    ...
    void writeUTCTime(Out<ostream> out)
    {
        ...
        out.get() << setw(4) << setfill('0') << date.year() << "-" << setw(2)
                  << setfill('0') << date.month().as_number() << "-" << setw(2)
                  << setfill('0') << date.day().as_number() << "T" << setw(2)
                  << setfill('0') << time.hours() << ":" << setw(2)
                  << setfill('0') << time.minutes() << ":" << setw(2)
                  << setfill('0') << time.seconds() << "." << setw(6)
