...
class Level final {
public:
    enum class Id { NONE, ERROR, WARN, INFO, VERBOSE, TRACE };
    ...
    static string getNameHelp() {
        ...
        for (auto levelIter = levelList.begin() + 1;
             levelIter != levelList.end();
             ++levelIter) {
            nameHelp += '|';
    ...
    static Level fromString(const string& name) {
        ...
        for (const auto& level : levelList) {
            ...
            if (compareString == nameLower) { return level; }
        }
...
enum class EntryType { Begin, End, Log };

inline const char* logEntryTypeToString(EntryType type) {
    if (type == EntryType::Log) { return "log"; }

    if (type == EntryType::Begin) { return "begin"; }
