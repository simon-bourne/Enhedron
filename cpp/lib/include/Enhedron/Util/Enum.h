...
/**
 * Convert a value to an enum safely. The enum must have "LAST_ENUM_VALUE" as
 * the last
 * enumerated value, and must be a given enum starting at 0 and being densely
...
{
    if (value >= 0 &&
        value <= static_cast<underlying_type_t<Enum>>(Enum::LAST_ENUM_VALUE))
    {
...
/**
 * Tag a type with an enumerated value to create a distinct type. Useful to
 * differentiate values of the same type.
 * For example, to stop first and last names being interchangeable in the type
 * system:
 *      enum class Name {
 *          FIRST,
 *          LAST
