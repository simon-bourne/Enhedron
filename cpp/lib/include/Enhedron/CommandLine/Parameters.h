...
class Arguments final : public NoCopy
{
    ...
    template <typename T>
    using isVector =
        is_same<T, vector<typename T::value_type, typename T::allocator_type>>;
        ...
        ParamTail&&... paramTail)
    {
        ...
        param.forEachName([&](const string& name) {
            ...
            paramValues.insert(
                paramValues.end(), newValues.begin(), newValues.end());
        });
        ...
        ParamTail&&... paramTail)
    {
        ...
        flag.forEachName(
            [&](const string& name) { flagValue |= setFlags.count(name) > 0; });
