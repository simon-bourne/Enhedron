...
struct Results : public NoCopy
{
    ...
    virtual void
    beginContext(const NameStack& contextStack, const string& name) = 0;
    virtual void endContext(
        const Stats& stats,
        const NameStack& contextStack,
        const string& name) = 0;

    virtual void beginGiven(const NameStack& context, const string& given) = 0;
    virtual void endGiven(
        const Stats& stats,
        const NameStack& context,
        const string& given) = 0;

    virtual void beginWhen(
        const NameStack& context,
        const string& given,
        const NameStack& whenStack,
        const string& when) = 0;
    virtual void endWhen(
        const Stats& stats,
        const NameStack& context,
        const string& given,
        const NameStack& whenStack,
...
class HumanResults final : public Results
{
        ...
        const NameStack& when)
    {
        ...
        auto startIndex =
            static_cast<vector<string>::difference_type>(whenWrittenDepth_);
...
public:
    ...
    virtual void finish(const Stats& stats) override
    {
        if (verbosity_ >= Verbosity::SUMMARY)
        {
            ...
            *output_ << "Totals: " << stats.tests() << " tests, "
