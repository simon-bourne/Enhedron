...
runTests(bool listOnly, string verbosityString, vector<string> pathList)
{
    ...
    for (auto& path : pathList)
    {
        ...
        pathRegexs.emplace_back(
            make_shared<vector<regex>>(move(pathComponentList)));
    }
...
inline int run(int argc, const char* argv[])
{
    ...
    return args.run(
        ...
        Option<string>(
            Name('v', "verbosity", "Set the verbosity"),
            "silent|summary|contexts|fixtures|sections|checks|checks_"
