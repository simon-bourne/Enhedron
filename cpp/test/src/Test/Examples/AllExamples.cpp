    ...
    t("examples",
      context(
          ...
          given("some constants to assert with", [](auto& check) {
              ...
              // Assertion example 3:
              check(
                  "`c` and `d` are provided for context",
                  VAR(a) == 0 && VAR(b) == 1,
                  VAR(c),
                  VAR(d));
              ...
              // Assertion example 6:
              check.template throws<runtime_error>(
                  "a runtime_error is thrown", VAR(throwRuntimeError)());
              // Assertion example 6 end.

              // Assertion example 7:
              check.throws(
                  "`a` and `b` are provided for context",
              ...
              // Assertion example 9:
              check.fail(
                  VAR("`a` and `b` are provided for context"), VAR(a), VAR(b));
              ...
              check(
                  "3 cubed is 27",
                  multiplyOverloadedProxy(VAR(three), 3, 3) == 27);
...
static Suite
    u("the root context",
      given(
          "a test can go here",
          [](auto& check) { check("`a` is one", VAR(a) == 1); }),
      context(
          "a context",
          given(
              "or a test can go here",
              [](auto& check) { check("`a` is one", VAR(a) == 1); }),
          context(
              "contexts can be nested to arbitrary depth",
