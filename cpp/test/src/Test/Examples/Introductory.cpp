    ...
    u("Util",
      ...
      given(
          ...
          [](auto& check) {
              ...
              check.when("we add an element", [&] {
                  ...
                  // This test will run twice. The first time, it will run the
                  // when
                  // block labelled when("we add a different element"), but skip
                  // the
                  // when block labelled when("we add the same element"). The
                  // second
                  // time it runs, it will do the inverse. There can be an
                  // arbitrary
                  // number of when blocks within each block, nested to an
                  // arbitrary
                  // depth.
      ...
      exhaustive(
          ...
          .given(
              ...
              [](Check& check, size_t initialSize, size_t resizeTo) {
      ...
      context(
          "we can also nest contexts",
          context(
              "to an arbitrary depth",
              given(
                  "an empty test to illustrate that tests can go here",
                  [](auto& check) {}))));
